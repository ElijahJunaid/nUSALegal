<template>
  <NuxtLayout name="default">
          ...
          <div class="card-body">
            ...
            <div v-else-if="courtType === 'scotus'" class="sub-menu-list">
              ...
              <button
                ...
                @keydown.enter="setVolume(vol.volume)"
                @keydown.space="setVolume(vol.volume)"
                class="btn"
                :class="{
                  'btn-primary': courtsStore.selectedVolume === vol.volume,
                  'btn-outline': courtsStore.selectedVolume !== vol.volume
                }"
                :aria-label="`Select Volume ${vol.volume}`"
                :aria-pressed="courtsStore.selectedVolume === vol.volume"
              >
                Volume {{ vol.volume }}
              </button>
            </div>
    ...
    <template #default>
          ...
          <div class="card-body">
            <div v-if="courtType === 'selection'" class="selection-screen">
              ...
              <div class="grid gap-6 md:grid-cols-2 max-w-4xl mx-auto">
                <button
                  ...
                  aria-label="Browse Supreme Court decisions"
                >
                ...
                <button
                  ...
                  @keydown.space="courtType = 'district'"
                  class="court-selection-card"
                  aria-label="View District Court records"
                >
                  <div class="text-6xl mb-4">⚖️</div>
            ...
            <div v-else-if="courtType === 'scotus'">
              ...
              <div v-else class="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
                <div
                  v-for="(courtCase, index) in courtsStore.filteredCases"
                  :key="index"
                ...
                >
                  <div class="card-body p-4 text-center">
                    <h3 class="text-lg font-bold text-center" style="text-align: center">
                      {{ courtCase.title }}
                    </h3>
                    <p class="text-sm mb-4 text-center text-base-content">
            ...
            <div v-else-if="courtType === 'district'">
              ...
              <div v-else class="grid gap-4 lg:grid-cols-1 xl:grid-cols-2">
                <div
                  v-for="(courtCase, index) in filteredDistrictCases"
                  :key="index"
      ...
      <LazyModal
        v-if="showCaseDetail && selectedCase"
        :show="showCaseDetail"
        max-width="max-w-4xl"
        ...
        <div class="case-detail-modal">
          ...
          <div class="bg-base-200 p-6 rounded-lg">
            <p class="text-lg text-center leading-relaxed">
              {{ selectedCase.ruling || selectedCase.description }}
            </p>
          </div>
        </div>
      </LazyModal>
      ...
      <LazyModal
        v-if="showDistrictCaseDetail && selectedDistrictCase"
        :show="showDistrictCaseDetail"
        max-width="max-w-4xl"
        ...
        <div class="case-detail-modal">
          <div class="text-center mb-6">
            ...
            <div class="flex items-center justify-center gap-4 mb-4 flex-wrap">
              ...
              <span class="text-sm">
                <strong>Case #:</strong>
                {{ selectedDistrictCase.caseNumber }}
              </span>
              <span class="text-sm">
                <strong>Date:</strong>
                {{ new Date(selectedDistrictCase.date).toLocaleDateString() }}
              </span>
            </div>
          </div>
          ...
          <div class="grid md:grid-cols-2 gap-4 mb-6">
            <div>
              <p class="text-sm mb-2">
                ...
                {{ selectedDistrictCase.plaintiff }}
              </p>
              <p class="text-sm mb-2">
                <strong>Defendant:</strong>
                {{ selectedDistrictCase.defendant }}
              </p>
              <p class="text-sm mb-2">
                <strong>Charge:</strong>
                {{ selectedDistrictCase.charge }}
              </p>
            </div>
            <div>
              <p class="text-sm mb-2">
                <strong>Judge:</strong>
                {{ selectedDistrictCase.judge }}
              </p>
              <p class="text-sm mb-2">
                <strong>Prosecutor:</strong>
                {{ selectedDistrictCase.prosecutor }}
              </p>
              <p class="text-sm mb-2">
                <strong>Defense Attorney:</strong>
                {{ selectedDistrictCase.defense }}
              </p>
            </div>
          </div>
...
const filteredDistrictCases = computed(() => {
  ...
  return districtCases.value.filter(
    c =>
