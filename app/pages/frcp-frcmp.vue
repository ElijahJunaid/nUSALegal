        ...
        <div class="card">
          <div class="card-body">
            <div class="sub-menu-list">
              <button
                @click="setSection('frcp')"
                @keydown.enter="setSection('frcp')"
                @keydown.space="setSection('frcp')"
                class="btn"
                :class="{
                  'btn-primary': selectedSection === 'frcp',
                  'btn-outline': selectedSection !== 'frcp'
                }"
                aria-label="View Federal Rules of Civil Procedure"
                :aria-pressed="selectedSection === 'frcp'"
              >
                FRCP
              </button>
              <button
                @click="setSection('frcmp')"
                @keydown.enter="setSection('frcmp')"
                @keydown.space="setSection('frcmp')"
                class="btn"
                :class="{
                  'btn-primary': selectedSection === 'frcmp',
                  'btn-outline': selectedSection !== 'frcmp'
                }"
                aria-label="View Federal Rules of Criminal Procedure"
                :aria-pressed="selectedSection === 'frcmp'"
              >
                FRCMP
              </button>
            </div>
          ...
          <div class="card-body">
            ...
            <div class="search-filter">
              ...
              <div class="form-control w-full md:w-80">
                ...
                >
                  <option value="all">
                    All {{ selectedSection === 'frcp' ? 'FRCP' : 'FRCMP' }}
                  </option>
...
const { loading, selectedSection, searchQuery, filteredFederalRules, filterList, filterType } =
  storeToRefs(FederalRulesStore)
