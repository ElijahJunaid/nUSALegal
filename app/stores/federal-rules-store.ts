...
export const useFederalRulesStore = defineStore('federal-rules', {
  ...
  getters: {
    filterList: state => {
      const data: FederalRulesGroup[] =
        state.selectedSection == 'frcp' ? state.frcp_group : state.frcmp_group

      return data.map(d => d.label)
    },
    filteredFederalRules: state => {
      let data: FederalRulesGroup[] =
        state.selectedSection == 'frcp' ? state.frcp_group : state.frcmp_group
  ...
  actions: {
    ...
    async fetchFRCP() {
      ...
      try {
        ...
        dError('Failed to fetch FRCP:', err)
      } finally {
        this.loading = false
      }
    },
    ...
    async fetchFRCMP() {
      ...
      try {
        ...
        dError('Failed to fetch FRCMP:', err)
      } finally {
        this.loading = false
      }
    }
